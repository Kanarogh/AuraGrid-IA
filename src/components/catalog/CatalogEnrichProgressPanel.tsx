...
export function CatalogEnrichProgressPanel({
  ...
  const percent =
    total > 0 ? Math.min(100, Math.round((index / total) * 100)) : undefined;
      ...
      <div className="space-y-3 px-4 py-3">
        <div>
          ...
          {total > 0 && (
            <div className="h-2 overflow-hidden rounded-full bg-ag-surface-3">
              <div
                className="h-full rounded-full bg-ag-accent transition-[width] duration-300 ease-out"
        ...
        <div className="flex items-start gap-2 text-xs">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10.5px] text-ag-muted mt-0.5">Um look por vez · ~5s entre peças</p>
          </div>
        </div>
      </div>
    </div>
